...
class S7I2Slave(Module, AutoCSR, AutoDoc):
      ...
      # One cache line is 8 32-bit words, need to always have enough space for one line or else
        ...
        if hasattr(pads, 'rx'):
            ...
            self.rx_conf = CSRStatus(description="Rx configuration",
                fields=[
                    CSRField("format", size=2, reset=frame_format.value, description="I2S sample format"),
        ...
        if hasattr(pads, 'tx'):
            ...
            if concatenate_channels:
                sample_msb = sample_width * 2
            else:
                sample_msb = sample_width
            ...
            txi2s.act("LEFT",
                If(~self.tx_ctl.fields.enable,
                    ...
                    NextValue(tx_pin, tx_buf[sample_msb - 1]),
